	...
	function Radisk(opt){
		...
		*/
		var r = function(key, val, cb){
			...
			if(val instanceof Function){
				...
				val = r.batch(key);
				if(u !== val){
					console.log(`returned ${val} from cache`);
					return cb(u, val);
				}
				if(r.thrash.at){
					val = r.thrash.at(key);
					if(u !== val){
						console.log((`returned ${val} from thrash`));
						return cb(u, val);
					}
				}
				...
				*/
				...
				/* Refactor into separate function a la r.thrash */
		...
		*/
		...
		r.read = function(key, cb){
			//console.log(`read called with ${key}`);
			...
			if(RAD){ // cache
				var val = RAD(key);
				if(u !== val){
					console.log(`returned ${key} with ${val} from cache`)
					return cb(u, val);
				}
			}
			...
			g.lex = function(file){
				...
				file = (u === file)? u : decodeURIComponent(file);
				//console.log(`actually reading file ${file}, with ${key}`)
				if(!file || file > key){
					if(tmp = q[g.file]){
