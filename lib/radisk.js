		...
		*/
		var r = function(key, val, cb){
			...
			if(val instanceof Function){
				...
				/* reading from disk needs to be added to a queue
				* Two cases to handle:
				* ONE - new Key comes in, no array at [key]
				* TWO - Key already is waiting for a read
				*/
			/*	if(r.readQueue[key] === u){//ONE
					r.readQueue[key] = [];
					r.readQueue[key].push(cb)
					r.readEd++
				} else {//TWO
					r.readQueue[key].push(cb)
					r.readEd++
				...
				if(r.readEd > opt.batch){
					r.readEd = 0;
					var batch = r.readQueue;
					r.readQueue = Object(null);
					Gun.obj.map(batch,function(node,key,t){
						for(var i =0;i<batch[key].length;i++){
							r.read(key,batch[key])
						}
					})
		...
		*/
		r.save = function(rad, cb){
			...
			s.find = function(tree, key){
				...
				s.start = key;
<<<<<<< HEAD
				//console.log(`assigned ${key} to s.start`);
				opt.store.list(s.lex);
=======
				r.list(s.lex);
>>>>>>> 3488e3cb2a35820eeca6d16e09666b755e4cb330
				return true;
			}
			s.lex = function(file){
...
>>>>>>> 3488e3cb2a35820eeca6d16e09666b755e4cb330
		...
		/* r.read should
		 - create a batch of keys to be read
		 - when the batch gets too big ->
		 - resolve the file in which they are kept (minimize calling list)
		 - call parse on the file, resolve each key from disk variable created
		 - sequentialy read instead of readFile
		 */
		...
		r.read = function(key, cb, next){
			...
			g.lex = function(file){
				//console.log(`reading file ${file}, with ${key} `)
				file = (u === file)? u : decodeURIComponent(file);
			...
			g.it = function(err, disk){
				...
				Gun.obj.map(disk, g.ack);//batch of keys per the file
			}
			g.ack = function(as){
				if(!as.ack){ return }
