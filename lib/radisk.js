;(function(){
	...
	function Radisk(opt){
		...
		var puff = Gun.__utils__.setTimeoutTurn || timediate, u;
		...
		var parse = Gun.__utils__.parseAsync || function(t,cb,r){ var u; try{ cb(u, JSON.parse(t,r)) }catch(e){ cb(e) } }
		var json = Gun.__utils__.stringifyAsync || function(v,cb,r,s){ var u; try{ cb(u, JSON.stringify(v,r,s)) }catch(e){ cb(e) } }
		/*
		...
		*/
		...
		r.save = function(key, data, cb, tag, DBG){
			...
			s.find = function(file){ var tmp;
				...
				//Gun.log.only.i && console.log('found', file);
			...
			s.mix = function(err, disk){
				...
				//Gun.log.only.i && console.log('mix', disk.Q);
			...
			s.write = function(){
				...
				delete r.disk[file];
				delete disk.tags;
				//Gun.log.only.i && console.log('write', file, disk, 'was saving:', key, data);
			...
			//Gun.log.only.i && console.log('save', key);
			r.find(key, s.find);
    }
    r.disk = {};
    r.one = {};
		...
		*/
		...
		r.write = function(file, rad, cb, o, DBG){
			...
			f.write = function(){
				...
				var S = +new Date;
				DBG && (DBG.wd = S);
				//Gun.log.only.i && console.log('add', file);
				r.find.add(file, function add(err){
					DBG && (DBG.wa = +new Date);
					if(err){ cb(err); return }
					//Gun.log.only.i && console.log('disk', file, text);
					opt.store.put(ename(file), text, function safe(err, ok){
						...
						//Gun.log.only.i && console.log('done', err, ok || 1, cb);
						cb(err, ok || 1);
			...
			//Gun.log.only.i && console.log('writing');
		...
		;(function(){
			...
			*/
			...
			r.parse = function(file, cb, raw, DBG){ var q;
				...
				p.map = function(){ // switch to Gun.__utils__.setTimeoutEach now?
