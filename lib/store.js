...
Gun.on('create', function(root){
    ...
    root.on('put', function(msg){
        ...
        var S = (msg._||'').PS || ((msg._||'').PS = +new Date);
        dare(soul+esc+key, {':': val, '>': state}, dare.one[id] || function(err, ok){
            LOG && Gun.log(S, +new Date - S, 'put');
            if(!err && count[id] !== lot.s){ console.log(err = "Disk count not same as ram count."); LOG && Gun.log(+new Date, lot.s - count[id], 'put ack != count') } delete count[id];
    ...
    root.on('get', function(msg){
        ...
        dare(key||'', function(err, data, info){
            ...
            if(LOG && (ST = +new Date - S) > 9){ Gun.log(S, ST, 'got prep time'); Gun.log(S, C, 'got prep #') } SPT += ST; C = 0; S = +new Date; (console.STAT||'').rgi = S; 
