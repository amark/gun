	...
	Gun.on('opt', function(root) {
		...
		function enqueueNode(soul, ctime) {
			if (nodes[soul] == true) { //The node already exists in the queue
				...
				if (index == -1) {
					DEBUG && console.err("Something happened and the node '" + soul + "' won't get garbage collection unless the value is updated agian");
					return;
				} else {
		...
		function GC(memRatio) {
			...
			if (curTime - memoryUpdate >= 5000) {
				DEBUG && console.log("|GC| %s | Current Memory Ratio: %d | Current Ram Usage %sMB | Nodes in Memory %s", new Date().toLocaleString(), round(memRatio, 2), round(ev.used, 2), Object.keys(root.graph || empty).length);
			...
			if (freed > 0)
        DEBUG && console.log("|GC| Removed %s nodes in %s seconds-----------------------------------------------------------------", freed, (Date.now() - curTime) * 0.001);
		}
