...
Gun.chain.group = function(obj, cb, opt){
	...
	opt = opt || {};
	cb = cb || function(){};
	gun = gun.put({}); // insert assumes a graph node. So either create it or merge with the existing one.
	var error, item = gun.chain().put(obj, function(err){ // create the new item in its own context.
		...
		gun.put(add, cb); // merge with the graph node.
	});
	return gun;
};