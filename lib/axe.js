...
function start(root){
	...
	function GET(msg){
		...
		var sub = (via.sub || (via.sub = new OurMap));
		if('string' == typeof (soul = msg.get['#'])){ ref = root.$.get(soul) }
		if('string' == typeof (tmp = msg.get['.'])){ has = tmp } else { has = '' }
		ref && (sub.get(soul) || (sub.set(soul, tmp = new OurMap) && tmp)).set(has, 1); // {soul: {'':1, has: 1}}
		if(!(ref = (ref||'')._)){ return fall(msg) }
		ref.asked = +new Date;
		(ref.route || (ref.route = new OurMap)).set(via.id, via); // this approach is not gonna scale how I want it to, but try for now.
	...
	GET.turn = function(msg, route, turn){
		...
		next = (objectMaps(route||opt.peers)).slice(turn = turn || 0);
		if(!next.length){
		...
		Gun.__utils__.setTimeoutEach(next, function(id){
	...
	root.on('create', function(){
		...
		root.on('put', function(msg){
			...
			Gun.__utils__.setTimeoutEach(objectMaps(route), function(id){ var peer, tmp;
				if(!(peer = route.get(id))){ return }
				if(!peer.wire){ route.delete(id); return } // bye!
				var sub = (peer.sub || (peer.sub = new OurMap)).get(soul);
				if(!sub){ return }
...
var from = Array.from;
function objectMaps(o){
	if(from && o instanceof Map){ return from(o.keys()) }
	if(o instanceof OurMap){ o = o.s }
	return Object.keys(o);
}
var OurMap;
if (typeof Map == 'function') OurMap = Map;
else {
	OurMap = function() {
		this.s = {}
	}
	OurMap.prototype.set = function(k,v){this.s[k]=v;return this};
	OurMap.prototype.get = function(k){return this.s[k]};
	OurMap.prototype.delete = function(k){delete this.s[k];return this};
}
