...
<<<<<<< HEAD
			//console.log(`Called radix with ${key} , ${val},${t}`);
=======
			key = ''+key;
			if(!t && u !== val){ 
...
>>>>>>> 7a8fceba071bcac55cbfb707d768cf603b764f18
			...
			//console.log(`l:${l},k = ${k},at = ${at},temp=${tmp}`);
			while(!(at = t[k]) && i < l){
				k += key[++i];
			}
			//console.log(`${k} put together`);
			if(!at){
			...
			if(i == l){
				//console.log(`i==l true -> u===val : ${u === val}`)
				//console.log(`mistery ${at[$]}`);
