...
;(function(){
	...
	var es_module = arg === 'es_module';
	if(es_module) { arg = 'gun'; }

	if(es_module) {
		rm('./es_module');
		mk('./es_module');
		// mk('./es_module/polyfill');
		mk('./es_module/adapters');
	} else if(arg === 'gun'){
		rm('./src');
		mk('./src');
		mk('./src/polyfill');
	...
	(function recurse(c){
		...
		var file = es_module ? path('es_module') : path(arg);
		if(!file){ return }

		code = code.replace(/\bUSE\(/g, 'require(');
		if(es_module) {
			code = code.replace(/var\s*(\w+)\s*=\s*require\(['"]([\w/.-_]+)['"]\)/g, "import $1 from '$2'");
			code = code.replace(/require\((['"][\w/.-_]+['"])\);/g, 'import $1;');
		}
