	...
	function Radisk(opt){
		...
		*/
		var r = function(key, data, cb){
			...
			//console.only(1, key, data);
			r.save(key, data, cb);
		}
		r.save = function(key, data, cb){
			var s = {key: key};
			s.find = function(file){ var tmp;
				//console.only(4, 'find', file);
				s.file = file || (file = opt.code.from);
				if(tmp = r.disk[file]){ s.mix(null, tmp); return }
				r.parse(file, s.mix);
			}
			s.mix = function(err, disk){
				...
				//console.only(7, key, data, disk.$, disk.Q);
				if(disk.Q){ disk.Q.push(cb); return } disk.Q = [cb];
				//console.only(6, key, data, disk.$);
			...
			s.write = function(){
				...
				delete disk.Q;
				delete r.disk[file];
				//console.only(8, s.file, q.length);
		...
		;(function(){
			...
			r.read = function(key, cb, o){
				...
				g.find = function(file){ var tmp;
					...
					if(tmp = r.disk[file]){ g.check(null, tmp); return }
				...
				g.get = function(err, disk, info){
					...
					if(!disk){ cb(null); return }
					disk.file || (disk.file = file);
					// ----------------------------
					var data = disk(key);
					if(u !== data){ cb(null, data); return }
					// ----------------------------
					return;
					var S; LOG && (S = +new Date);
			...
			*/
			...
			r.parse = function(file, cb, raw){ var q;
				...
				p.read = function(err, data){ var tmp;
					...
					delete Q[file];
					//console.only(5, 'parse', file, data, q.length);
