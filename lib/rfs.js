function Store(opt){
	...
	store.get = function(file, cb){
		...
		if(true){
			...
			store.total += store.batch.length;
			console.log(`read start: ${store.counter}, ${store.total}`);
			var arr = store.batch;
			store.batch=[];
			var item = arr.shift();
			fs.readFile(opt.file+'/'+item.f, function(err,data){
				console.log(item);
				store.counter--;
				...
				fs.readFile(opt.file+'/'+item.f, function(err, data){
					console.log(item);
					store.counter--;
					console.log('read finised: ', store.counter, store.total);
					if(err){
						if('ENOENT' === (err.code||'').toUpperCase()){
							return item.cb(null);
						}
