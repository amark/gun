(function() {
  ...
  USE(function(module) {
    ...
    if (SEA.window) {
      ...
      api.random = len =>
        Buffer.from(
          api.crypto.getRandomValues(new Uint8Array(Buffer.alloc(len)))
        );
    }
    if (!api.crypto) {
      try {
      ...
      (async (cb, opt) => {
        try {
          ...
          var sa = await shim.subtle
            .generateKey(S.ecdsa.pair, true, ["sign", "verify"])
            .then(async keys => {
              // privateKey scope doesn't leak out from here!
              //const { d: priv } = await shim.subtle.exportKey('jwk', keys.privateKey)
              var key = {};
              key.priv = (await shim.subtle.exportKey(
                "jwk",
                keys.privateKey
              )).d;
          ...
          try {
            var dh = await ecdhSubtle
              .generateKey(S.ecdh, true, ["deriveKey"])
              .then(async keys => {
                // privateKey scope doesn't leak out from here!
                var key = {};
                key.epriv = (await ecdhSubtle.exportKey(
                  "jwk",
                  keys.privateKey
                )).d;
      ...
      (async (data, pair, cb, opt) => {
        try {
          ...
          var sig = await (shim.ossl || shim.subtle)
            .importKey("jwk", jwk, S.ecdsa.pair, false, ["sign"])
            .then(key =>
        ...
        f <= SEA.opt.fallback
          ? shim.Buffer.from(
              await shim.subtle.digest(
                { name: "SHA-256" },
                new shim.TextEncoder().encode(S.parse(json.m))
              )
            )
      ...
      (async (key, pair, cb, opt) => {
        try {
          ...
          var derived = await ecdhSubtle
            .importKey(...privKeyData, false, ["deriveKey"])
            .then(async privKey => {
              // privateKey scope doesn't leak out from here!
              var derivedKey = await ecdhSubtle.deriveKey(
                props,
                privKey,
  ...
  USE(function(module) {
      ...
      (async pub => {
        try {
          ...
          const id = Buffer.concat([
            Buffer.from([0x99, pb.length / 0x100, pb.length % 0x100]),
            pb
          ]);
    ...
    var Gun =
      (SEA.window || {}).Gun ||
      USE((typeof common == "undefined" ? "." : "") + "./gun", 1);
    Gun.SEA = SEA;
    SEA.GUN = SEA.Gun = Gun;
  ...
  USE(function(module) {
    ...
    User.prototype.create = function(alias, pass, cb, opt) {
      ...
      act.g = function(auth) {
        ...
        setTimeout(function() {
          // we should be able to delete this now, right?
          cat.ing = false;
    ...
    User.prototype.auth = function(alias, pass, cb, opt) {
      ...
      opt = opt || {};
      var pair =
        alias && (alias.pub || alias.epub)
          ? alias
          : pass && (pass.pub || pass.epub)
          ? pass
          : null;
      var act = {},
        u;
      ...
      act.g = function(pair) {
        ...
        try {
          if (
            pass &&
            !Gun.obj.has(Gun.obj.ify(cat.root.graph["~" + pair.pub].auth), ":")
          ) {
  ...
  USE(function(module) {
    ...
    function security(msg) {
      ...
      if (msg.put) {
        ...
        each.way = function(val, key) {
          ...
          if (
            "~" === soul.slice(0, 1) &&
            2 === (tmp = soul.slice(1)).split(".").length
          ) {
        ...
        each.pub = function(val, key, node, soul, pub, user) {
          ...
          if (Gun.is(msg.$) && user && user.is && pub === user.is.pub) {
            SEA.sign(
              SEA.opt.prep((tmp = SEA.opt.parse(val)), key, node, soul),
              user._.sea,
              function(data) {
                var rel;
                if (u === data) {
    ...
    SEA.opt.pack = function(d, k, n, s) {
      ...
      var meta = Gun.obj.ify(d) || noop,
        sig = meta["~"];
      return sig
        ? {
            m: { "#": s, ".": k, ":": meta[":"], ">": Gun.state.is(n, k) },
            s: sig
          }
        : d;
    };
