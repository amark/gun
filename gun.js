...
(function() {
  ...
  require(function(module) {
    ...
    var Type = {};
    Type.fns = Type.fn = {is: function(fn) { return (Boolean(fn) && fn instanceof Function); }};
  ...
  Type.obj.empty = function(o, n) {
    ...
    Type.time = {};
    Type.time.is = function(t) { return t ? t instanceof Date : (Number(new Date().getTime())); };
  ...
  require(function(module) {
    ...
    function Scope() {
      function On(tag, arg, as, eas, skip) {
        ...
        if (!arg) {
          if (arguments.length === 1) { // Performance drops significantly even though `arguments.length` should be okay to use.
            return on;
          }
        }
      ...
      var act = this, on = act.on, halt = {
          resume: function(arg) {
            ...
            act.ctx.on(act.tag, (arguments.length ?
						arguments.length === 1 ? arg : Array.prototype.slice.call(arguments)
  ...
  function map(v, f) { // we invert this because the way we check for this is via a negation.
    ...
    (function() {
      ...
      function map(v, f) { var o = this.opt, tmp, u; // iterate over each field/value.
        if (o.map) {
          tmp = o.map.call(this.as, v, String(f), o.node);
          if (u === tmp) {
            obj_del(o.node, f);
          } else
  ...
  require(function(module) {
    ...
    (function() {
      ...
      Gun.HAM.union = function(vertex, node, opt) {
        ...
        opt = num_is(opt) ? {machine: opt} : {machine: (Number(new Date()))};
  ...
  return gun;
				...
				if (num_is(lex)) {
  return back.get(String(lex), cb, opt);
} else
				if (tmp = lex.soul) {
  if (!(gun = path[tmp])) {
    gun = cache(tmp, back);
  ...
  return;
      ...
      function proxy(at, ev) { var link = this;
        link.ran = true;
        if (link.sub === false) { return ev.off(); } // will this auto clean itself up?
    ...
    Gun.chain.path = function(field, cb, opt) {
      ...
      return back.get(String(field), cb, opt);
    }
    ...
    r.ws = function(opt, cb, req) {
      ...
      if (ws = r.ws.peers[opt.base]) {
        ...
        if (!opt.out && !ws.cbs[req.headers['ws-rid']]) {
          ws.cbs[req.headers['ws-rid'] = 'WS' + (Number(new Date())) + '.' + Math.floor((Math.random() * 65535) + 1)] = function(err, res) {
  ...
  require(function(module) {
    ...
    Gun.on('put', function(at) {
      ...
      if (opt.websocket === false || (at.opt && at.opt.websocket === false)) { return; }
      var msg = {
        '#': at['#'] || Gun.text.random(9), // msg ID
        '$': at.put // msg BODY
      };
