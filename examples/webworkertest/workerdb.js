;(function(){
  ...
  /* UNBUILD */
	...
	;USE(function(module){
		...
		;(function(){
			...
			Gun.on._get = function(msg, gun){
				...
				if(has){
					...
					node = Gun.self? Gun.obj.copy(node) : node; // HNPERF: If !browser bump Performance? Is this too dangerous to reference root graph? Copy / shallow copy too expensive for big nodes. Gun.obj.to(node); // 1 layer deep copy // Gun.obj.copy(node); // too slow on big nodes
				}
		...
		if(typeof self !== "undefined"){ (self.GUN = self.Gun = Gun).self = self }
		try{ if(typeof MODULE !== "undefined"){ MODULE.exports = Gun } }catch(e){}
		module.exports = Gun;

		(Gun.self||'').console = (Gun.self||'').console || {log: function(){}};
	...
	;USE(function(module){
		...
		try{store = (Gun.self||noop).localStorage}catch(e){}
		if(!store){
	...
	;USE(function(module){
		...
		Gun.on('opt', function(root){
			...
			var env;
			if(typeof self !== "undefined"){ env = self }
...
;(function(){
    ...
    r.tags = {};
		...
		*/
		...
		try{ !Gun.self && require('./radmigtmp')(r) }catch(e){}
...
var Gun = (typeof self !== "undefined")? self.Gun : require('../gun');
...
Gun.on('create', function(root){
    ...
    var Radisk = (Gun.self && Gun.self.Radisk) || require('./radisk');
...
;(function(){
  ...
  if(typeof self !== "undefined"){
    (Store.self = self).RindexedDB = Store;
  } else {
    try{ module.exports = Store }catch(e){}
  }

  try{
    var Gun = Store.self.Gun || require('../gun');
    Gun.on('create', function(root){
      this.to.next(root);
...
;(function(){
  ...
  ;USE(function(module){
    ...
    if(typeof self !== "undefined"){ module.self = self }

    var tmp = module.self || module;
    var SEA = tmp.SEA || {};

    if(SEA.self = module.self){ SEA.self.SEA = SEA }
  ...
  ;USE(function(module){
    ...
    if(SEA.self){
      api.crypto = self.crypto || self.msCrypto
      api.subtle = (api.crypto||o).subtle || (api.crypto||o).webkitSubtle;
      api.TextEncoder = self.TextEncoder;
      api.TextDecoder = self.TextDecoder;
  ...
  ;USE(function(module){
    ...
    var Gun = (SEA.self||{}).Gun || USE((typeof MODULE == "undefined"?'.':'')+'./gun', 1);
    Gun.SEA = SEA;
    SEA.GUN = SEA.Gun = Gun;
  ...
  ;USE(function(module){
    ...
    User.prototype.auth = function(alias, pass, cb, opt){
      ...
      act.g = function(pair){
        ...
        // if(SEA.self && ((gun.back('user')._).opt||opt).remember){
        //   // TODO: this needs to be modular.
        //   try{var sS = {};
        //   sS = self.sessionStorage;
        //   sS.recall = true;
        //   sS.alias = alias;
        //   sS.tmp = pass;
        //   }catch(e){}
